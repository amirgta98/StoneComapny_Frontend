...
export function ManagerSidebar({
  ...
  return (
    <aside
      ...
      aria-label="منوی ناوبری مدیریت کارخانه سنگ"
    >
      {/* Top Header / Factory Brand */}
      <div>
        <div className="flex items-center gap-3 border-b border-border/70 px-5 py-4">
          <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-amber-700 to-amber-950 text-white shadow-sm">
      ...
      {/* Pinned Bottom Area: Tenant & Manager info */}
      <div className="border-t border-border/70 p-3 bg-secondary/30">
        <div className="flex items-center justify-between gap-2 rounded-xl bg-card p-2.5 border border-border/60 shadow-2xs">
          <div className="min-w-0 flex-1">
            ...
            <p className="text-[10px] text-muted-foreground truncate mt-0.5 font-mono" dir="ltr">
              {data.tenantId} (MANAGER)
            </p>
          </div>
          <button
            onClick={() => void logout()}
            className="flex h-8 w-8 items-center justify-center rounded-lg text-muted-foreground hover:bg-destructive/10 hover:text-destructive transition-colors"
            title="خروج از حساب کارخانه"
          >
